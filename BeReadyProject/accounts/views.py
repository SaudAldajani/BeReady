...
def register_human_resource(request : HttpRequest):
    ...
    if request.method == "POST":
        ...
        hr_profile = HumanResourceProfile(user=new_hr, group="HR", desceiption=request.POST["desceiption"],price=request.POST["price"],image=request.FILES.get('image', 'default.jfif'))
        hr_profile.save()
        return redirect("BeReady:home")
